...
1. **Active projects and epics**: `search_keyword("status:active")` and `list_entities` filtered to type `project` and `epic`. Note what work is already planned or in flight — do not duplicate it.
...
**Check against active work.** For each theme, verify it is not already covered by an active epic or project in the KG. If it is partly covered, note what remains.

---

## Phase 5: Create an Initiative Brief

Before creating epics, write a brief summary of what this planning cycle found and decided. This provides continuity for the next planning cycle.
...
## Phase 6: Create Project Epics as Beads

For each prioritized theme, create an epic bead:
...
**Epic title format:** Start with a verb. State the outcome, not the task.
...
- Sentence 1: What problem this solves and for whom
- Sentence 2: What the affected modules/files are
- Sentence 3: What done looks like (the acceptance signal)

Limit to 3-5 epics per planning cycle. Quality over quantity — a well-scoped epic the team can actually execute is more valuable than ten vague ones that sit untouched.

---

## Phase 7: Write Architectural Contracts to the KG

For each epic, write the constraints that engineers must follow when implementing it. These are the pre-conditions for the batch — engineers will read them before starting.
...
add_observations([{
  ...
  content: "CONSTRAINT: [specific rule engineers must follow]. Rationale: [why this constraint exists]. Applies to: [epic title or bead ID].",
  type: "constraint",
  confidence: 0.9
}])
```
...
Write 1-3 constraints per epic. More than that suggests the epic is too large or too vague.
...
3. **Roadmap updates**: If the epics created advance a strategic goal, link them:
   ```
   add_relationships([{
     source: "<epic entity>",
...
- **Read before writing.** Never create epics without first reading the KG. Duplicate epics are noise.
...
- **3-5 epics maximum.** A planning cycle that produces 10 epics has not prioritized — it has delegated the prioritization problem downstream.
