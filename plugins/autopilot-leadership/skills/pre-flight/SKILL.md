---
name: pre-flight
description: This skill should be used when the CTO reviews a batch of beads before engineers start implementation. Produces architectural contracts — constraints stored in the KG that engineers must follow.
user-invocable: true
---

# Pre-Flight Review

You review a batch of beads before engineers begin implementation. Your job is to detect conflicts between beads, identify constraint violations, determine ordering dependencies, and write architectural contracts to the KG so engineers start with the right guardrails.
...
## Phase 1: Read the Batch

List the beads in this batch:
...
Build a matrix: rows are beads, columns are modules. Mark which beads touch which modules. Any module column with more than one mark is a potential conflict zone.
...
With the batch matrix and KG context in hand, check for two categories of conflicts:
...
- **Merge them**: the two beads should be combined into one, since they are solving the same problem. Flag this for the Director to reconsider the batch structure.
...
add_observations([{
  ...
  content: "CONSTRAINT [batch:<batch-id>]: <specific rule for engineers implementing beads in this batch>. Applies to: <bead title(s)>. Rationale: <why this constraint exists>.",
  type: "constraint",
  confidence: 0.9
}])
```
...
Write contracts to the module entity so they persist for future batches, not just this one. Contracts accumulate knowledge — they are not disposable.

---

## Phase 6: Write the Pre-Flight Report

After all contracts are written, write a summary observation on the batch entity:

```
add_observations([{
  entity: "<batch entity or epic entity>",
  content: "PRE-FLIGHT COMPLETE [batch:<batch-id>]: Reviewed <N> beads. Conflicts: <list or 'none'>. Ordering requirements: <list or 'none'>. Contracts written: <count>. Notes: <any issues that need Director attention>.",
  type: "pre-flight",
  confidence: 1.0
}])
```

If conflicts were found that require batch restructuring, flag them clearly so the Director can act before engineers start. A batch with unresolved structural conflicts should not proceed.
