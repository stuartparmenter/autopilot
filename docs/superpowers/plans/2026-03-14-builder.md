...
**Goal:** Create the `autopilot-builder` plugin (agent + implement-task skill) and `src/builder.ts` (spawn management with slots, timeouts, sandbox) so that tasks from beads can be picked up and built by sandboxed Sonnet agents in isolated worktrees.

**Architecture:** A builder manager in `src/builder.ts` exposes `spawnBuilder(taskId)` and manages a slot pool. Each builder is an Agent SDK `query()` call with the `autopilot-builder:builder` agent, sandboxed in a worktree with beads MCP, gk MCP, and GitHub MCP. The builder agent follows the `implement-task` skill (adapted from v2's `implement-bead`) for its full lifecycle: claim → worktree → implement → PR → update beads.
...
### Task 1: Create the `autopilot-builder` plugin scaffold

**Files:**
- Create: `plugins/autopilot-builder/.claude-plugin/plugin.json`
- Create: `plugins/autopilot-builder/agents/builder.md`
...
```json
{
  ...
  "version": "0.3.0",
  "description": "Builder agent for implementing tasks — claims from beads, works in worktrees, creates PRs"
}
```
...
---
name: builder
description: Builder agent. Use for implementing tasks from beads — claims the task, works in an isolated worktree, creates a PR, and writes observations to gk.
model: sonnet
color: blue
...
You are a builder implementing a task tracked in beads. You work in an isolated worktree and your goal is to produce a clean PR that satisfies the task's acceptance criteria.

Your workflow is defined by the `/implement-task` skill. Run it immediately when you start.

## Core Principles

1. **Claim before anything.** The claim is atomic. If it fails, stop — another builder owns this task.
...
Run: `ls plugins/autopilot-builder/.claude-plugin/plugin.json plugins/autopilot-builder/agents/builder.md`
Expected: Both files exist.

- [ ] **Step 4: Commit**

```bash
git add plugins/autopilot-builder/
git commit -m "feat: create autopilot-builder plugin with builder agent"
```
...
**Files:**
- Create: `plugins/autopilot-builder/skills/implement-task/SKILL.md`

Adapted from v2's `implement-bead` skill. Simplified: no merge-slot acquisition, no kg-extract subagent (builder uses gk-conventions directly), no gate/auto-merge pattern (orchestrator handles post-PR lifecycle in Plan 3).
...
name: implement-task
description: >-
  Full lifecycle for implementing a task from beads. Use when a builder agent
...
If the claim fails, **stop immediately**. The task belongs to another builder.
...
- Observations from prior builder sessions or planning cycles
...
```bash
git add plugins/autopilot-builder/skills/implement-task/SKILL.md
git commit -m "feat: add implement-task skill for builder lifecycle"
```

---

## Chunk 2: Builder Manager (`src/builder.ts`)

### Task 3: Create builder manager with slot pool

**Files:**
- Create: `src/builder.ts`
- Test: `src/builder.test.ts` (create)

The builder manager handles spawning builder agents, managing concurrency slots, and tracking active builders.
...
describe("BuilderManager", () => {
  test("creates with configured max slots", () => {
    const manager = new BuilderManager({ maxParallel: 3, projectPath: "/tmp" });
    expect(manager.availableSlots).toBe(3);
    expect(manager.activeCount).toBe(0);
  });

  test("hasAvailableSlot returns true when slots available", () => {
    const manager = new BuilderManager({ maxParallel: 2, projectPath: "/tmp" });
    expect(manager.hasAvailableSlot()).toBe(true);
  });

  test("tracks active builders", () => {
    const manager = new BuilderManager({ maxParallel: 2, projectPath: "/tmp" });
    // We can't easily test spawn without the SDK, but we can test the tracking
    expect(manager.activeCount).toBe(0);
...
- [ ] **Step 3: Implement BuilderManager**
...
export class BuilderManager {
  ...
  private active = new Map<string, ActiveBuilder>();
  ...
  async spawnBuilder(
    ...
    try {
      const result = await this.runBuilder(taskId, controller, onActivity);
      return result;
    } finally {
  ...
  abortAll(): void {
    for (const builder of this.active.values()) {
      builder.controller.abort();
    }
  }
  ...
  private async runBuilder(
    ...
    const plugins: SdkPluginConfig[] = [
      ...
      { type: "local", path: resolve(AUTOPILOT_ROOT, "plugins/autopilot-builder") },
    ];

    const prompt = [
      `# Build Task: ${taskId}`,
      "",
      `You are a builder agent. Your task ID is \`${taskId}\`.`,
      "",
...
Run: `bun test src/builder.test.ts`
Expected: PASS — 3 slot management tests pass.
...
git commit -m "feat: add BuilderManager with slot pool and SDK query setup"
```

---

### Task 4: Add timeout support to BuilderManager
...
```typescript
// Add to src/builder.test.ts
describe("BuilderManager timeouts", () => {
  test("default timeout is 60 minutes", () => {
    const manager = new BuilderManager({ maxParallel: 1, projectPath: "/tmp" });
    // Access via getter or check config
...
- [ ] **Step 2: Add timeout logic**

In `BuilderManager`:
- Add a `get timeoutMs()` getter that returns `this.config.timeoutMs ?? 60 * 60 * 1000`
- In `runBuilder`, set up a timeout that calls `controller.abort()` after `this.timeoutMs`:
...
```bash
git add src/builder.ts src/builder.test.ts
git commit -m "feat: add timeout support to BuilderManager"
```

---

### Task 5: Verify end-to-end typecheck and lint
...
```bash
git add src/ plugins/
git commit -m "chore: fix lint/format issues from builder implementation"
```
